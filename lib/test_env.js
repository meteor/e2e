    ...
    return new wd.asserters.Asserter(function (el, cb) {
      ...
      Fiber(function () {
        ...
        try {
          ...
          // due to network connection latency, when we execute element
          // methods in the asserter, the element could have been redrawn
          // by Blaze, thus causing a stale element reference error in
          // Selenium. In that case we simply return false and let wd poll again
          // to get the reference to the new element.
          // 
          // http://docs.seleniumhq.org/exceptions/stale_element_reference.jsp
          if (e.toString().indexOf('StaleElementReference') > -1) {
...
exports.init = function (options, callback) {
  ...
  browser.focusMainWindow = function () {
    ...
    if (browser.name.match(/ie/)) {
      // Fix Selenium IE-only bug: after switching window, the first click serve
      // as the "focus" click and won't trigger click events, so we make that
      // click here to make sure subsequent clicks work as intended.
      browser.find('body').click();
    }
  };
  ...
  // add this so we know what browser we are in
  ...
  browser.init(browserOptions, function () {
    ...
    if (!options.local) {
      var sauceLabsUrl = "https://saucelabs.com/tests/" + browser.sessionID;
      console.log("Follow this browser run on Sauce Labs: " + sauceLabsUrl);

      // Save the Sauce Labs URL so that it can be printed in the failure banner
      browser.sauceLabsUrl = sauceLabsUrl;
    }
